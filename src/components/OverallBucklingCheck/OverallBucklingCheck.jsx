...
const OverallBucklingCheck = () => {
  ...
  useEffect(() => {
    ...
    const { Tflange, A, ry, Sy, Zx, Zy } = selectedSection;
  ...
  return (
    <div className="bg-white p-4 rounded shadow">
      ...
      {bucklingResults ? (
        <div>
          <p>
            <strong>Design Strength (py):</strong>{" "}
            {bucklingResults.py.toFixed(2)} MPa
          </p>
          <p>
            <strong>Effective Length (LE):</strong>{" "}
            {bucklingResults.LE.toFixed(2)} m
          </p>
          <p>
            <strong>Slenderness Ratio (λ):</strong>{" "}
            {bucklingResults.λ.toFixed(2)}
          </p>
          <p>
            <strong>Compression resistance (Pc):</strong>{" "}
            {bucklingResults.Pc.toFixed(2)}
          </p>
          <p>
            <strong>Equivalent Slenderness (λLT):</strong>{" "}
            {bucklingResults.λLT.toFixed(2)}
          </p>
          <p>
            <strong>Buckling Resistance Moment (Mbs):</strong>{" "}
            {bucklingResults.Mbs.toFixed(2)} kNm
          </p>
          <p>
            <strong>Overall Buckling Check:</strong>{" "}
            {bucklingResults.OverallBucklingCheck.toFixed(2)}
          </p>
        </div>
      ) : (
        <p className="text-red-500">
          No results available. Please check your inputs.
        </p>
      )}
    </div>
  );
